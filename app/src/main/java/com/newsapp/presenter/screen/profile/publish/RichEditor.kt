...
import jp.wasabeef.richeditor.RichEditor.AfterInitialLoadListener
import jp.wasabeef.richeditor.RichEditor.EditorWebViewClient
import jp.wasabeef.richeditor.RichEditor.OnDecorationStateListener
...
import java.net.URLEncoder
    ...
    class RichEditor @SuppressLint("SetJavaScriptEnabled") constructor(
        ...
        init {
            ...
            loadUrl(RichEditor.Companion.SETUP_HTML)
        ...
        private fun callback(text: String) {
            mContents = text.replaceFirst(
                RichEditor.Companion.CALLBACK_SCHEME.toRegex(),
                ""
            )
        ...
        private fun stateCheck(text: String) {
            val state: String = text.replaceFirst(
                RichEditor.Companion.STATE_SCHEME.toRegex(),
                ""
            ).uppercase()
            val types: MutableList<RichEditor.Type> = ArrayList()
            for (type in RichEditor.Type.values()) {
        ...
        protected inner class EditorWebViewClient : WebViewClient() {
            override fun onPageFinished(view: WebView, url: String) {
                isReady = url.equals(
                    RichEditor.Companion.SETUP_HTML,
                    ignoreCase = true
                )
            ...
            override fun shouldOverrideUrlLoading(view: WebView, url: String): Boolean {
                ...
                if (TextUtils.indexOf(
                        url,
                        RichEditor.Companion.CALLBACK_SCHEME
                    ) == 0
                ) {
                    callback(decode)
                    return true
                } else if (TextUtils.indexOf(
                        url,
                        RichEditor.Companion.STATE_SCHEME
                    ) == 0
                ) {
                    stateCheck(decode)
                    return true
